...
public class BlockNetherBrickOven extends BlockContainer
{
	...
	public static void updateBlockState(boolean lit, World world, int x, int y, int z)
	{
		...
		if (lit)
		{
			world.setBlock(x, y, z, RegisterBlocks.netherBrickOvenLit);
		}
		else
		{
			world.setBlock(x, y, z, RegisterBlocks.netherBrickOven);
		}
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random)
	{
		if (isLit2)
		{
			...
			float f = x + 0.5f;
			float f1 = y + random.nextFloat() * 8.0f / 16.0f;
			float f2 = z + 0.5f;

			float f3 = 0.5f;
			float f4 = random.nextFloat() * 0.3f - 0.15f;
