...
public class BlockEnderSmelter extends BlockContainer
{
	...
	public BlockEnderSmelter(boolean lit)
	{
		...
		if (lit)
		{
			setBlockName("enderSmelterLit");
			this.setLightLevel(0.875F);
		}
		else
		{
			setBlockName("enderSmelter");
	...
	public static void updateBlockState(boolean lit, World world, int x, int y, int z)
	{
		...
		if (lit)
		{
			world.setBlock(x, y, z, RegisterBlocks.enderSmelterLit);
		}
		else
		{
			world.setBlock(x, y, z, RegisterBlocks.enderSmelter);
		}
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random)
	{
		if (isLit2)
		{
			...
			if (dir == 4)
			{
				...
				world.spawnParticle("portal", f - f3, f1, f2 + f4, 0.0F, 0.0F, 0.0F);
			}
			else if (dir == 5)
			{
				world.spawnParticle("smoke", f + f3, f1, f2 + f4, 0.0F, 0.0F, 0.0F);
			...
			else if (dir == 2)
			{
				...
				world.spawnParticle("portal", f + f4, f1, f2 - f3, 0.0F, 0.0F, 0.0F);
			}
			else if (dir == 3)
			{
				world.spawnParticle("smoke", f + f4, f1, f2 + f3, 0.0F, 0.0F, 0.0F);
	...
	{
		player.openGui(ModMain.modInstance, BBGuiHandler.ENDERSMELTER_ID, world, x, y, z);
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack)
	{
		...
		{
			((TileEntityEnderSmelter)world.getTileEntity(x, y, z)).furnaceName(stack.getDisplayName());
		}
	}
	...
	{
		blockIcon = iconregister.registerIcon(ModMain.MODID + ":enderSmelterSide");
		front = iconregister
				.registerIcon(isLit2 ? ModMain.MODID + ":enderSmelterLit" : ModMain.MODID + ":enderSmelterUnlit");
		top = iconregister.registerIcon(ModMain.MODID + ":enderSmelterTop");
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta)
	{
		if (!isLit)
		{
			TileEntityEnderSmelter smelter = (TileEntityEnderSmelter)world.getTileEntity(x, y, z);
			if (smelter != null)
			{
